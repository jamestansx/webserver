...
def test_client():
    ...
    app.config.update({
        "TESTING": True,
        "SQLALCHEMY_DATABASE_URI" : 'sqlite:///' + os.path.join(BASEDIR, 'webserver_test.db'),
...
def login_default_user_form(test_client, init_db, new_user):
    res = test_client.post(
            "/users/login", data=dict(username="user01", password="1234"), follow_redirects=True
          )
    return res
